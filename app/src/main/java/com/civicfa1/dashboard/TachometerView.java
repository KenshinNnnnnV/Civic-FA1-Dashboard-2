...
final class TachometerView extends View {
    ...
    @Override protected void onDraw(Canvas c){super.onDraw(c);float w=getWidth(),h=getHeight();float cx=w/2f,cy=h*.47f,r=Math.min(w,h)*.40f;arc.set(cx-r,cy-r,cx+r,cy+r);
        ...
        if(rpmValid&&!Float.isNaN(rpm)){
            // The live indicator uses the exact same arc bounds as the neutral scale above.
            // One uninterrupted green sweep keeps the fill even at every RPM and cannot drift
            // away from the tachometer band; red/yellow segments are intentionally not used.
            float clamped=Math.max(0,Math.min(8000,rpm));
            float sweep=(clamped/8000f)*270f;
            p.setColor(Ui.GREEN);
            c.drawArc(arc,135f,sweep,false,p);
        }
